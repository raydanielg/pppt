...
export default function Notes({ notes, categories = [] }) {
    ...
    const rows = useMemo(() => notes?.data || [], [notes]);
    ...
    const submit = (e) => {
        e.preventDefault();
        form.post(route('admin.library.notes.store'), {
            forceFormData: true,
            preserveScroll: true,
            onSuccess: () => form.reset('title', 'description', 'library_category_id', 'file'),
        });
    };

    const del = (id) => {
        if (!confirm('Delete these notes?')) return;
        router.delete(route('admin.library.notes.destroy', id), { preserveScroll: true });
    };

    return (
        <AdminLayout title="Library - Notes">
            <Head title="Admin - Library Notes" />
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                <div className="rounded-2xl border border-gray-200 bg-white p-5 lg:col-span-1">
                    <div className="text-sm font-black text-gray-900">Upload Notes</div>
                    <div className="mt-1 text-xs text-gray-500">Upload PDF/Word/PPT notes.</div>
                    ...
                    <form onSubmit={submit} className="mt-4 space-y-4">
                        ...
                        <div>
                            <label className="text-xs font-black uppercase tracking-widest text-gray-500">File</label>
                            <div className="mt-2 rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-4">
                                ...
                                />
                                {form.errors.file ? <div className="mt-1 text-xs font-bold text-rose-600">{form.errors.file}</div> : null}
                            </div>
                        </div>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5 lg:col-span-2">
                    <div className="text-sm font-black text-gray-900">All Notes</div>
                    <div className="mt-1 text-xs text-gray-500">Latest uploads.</div>

                    <div className="mt-4 space-y-3">
                        {rows.length ? (
                            rows.map((n) => (
                                <div key={n.id} className="rounded-2xl border border-gray-200 bg-white p-4 flex items-center justify-between gap-4">
                                    <div className="min-w-0">
                                        <div className="text-xs font-black text-gray-900 truncate">{n.title}</div>
                                        <div className="mt-1 text-[10px] font-black uppercase tracking-widest text-gray-500">
                                            {n.library_category?.name ?? n.category ?? '—'} {n.file_size ? `• ${n.file_size}` : ''}
                                        </div>
                                        {n.description ? <div className="mt-1 text-xs text-gray-500 line-clamp-1">{n.description}</div> : null}
                                    </div>
                            ...
                            <div className="rounded-2xl border border-gray-200 bg-gray-50 p-10 text-center">
                                ...
                                <div className="mt-1 text-xs text-gray-500">Upload your first notes file.</div>
                            </div>
                        )}
                    </div>
