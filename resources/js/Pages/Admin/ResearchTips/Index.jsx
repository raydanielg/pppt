...
export default function Index({ tips, filters }) {
    ...
    const submitSearch = (e) => {
        e.preventDefault();
        router.get(route('admin.research-tips.index'), { q }, { preserveScroll: true, preserveState: true });
    };

    const del = (id) => {
        if (!confirm('Delete this research tip?')) return;
        router.delete(route('admin.research-tips.destroy', id), { preserveScroll: true });
    };

    return (
        <AdminLayout title="Research Tips">
            <Head title="Admin - Research Tips" />
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="min-w-0">
                        <div className="text-sm font-black text-gray-900">All Research Tips</div>
                        <div className="text-xs text-gray-500">Manage research content published on the platform.</div>
                    </div>

                    <div className="flex items-center gap-2">
                        <Link
                            href={route('admin.research-tip-categories.index')}
                            className="rounded-xl border border-gray-200 bg-white px-3 py-2 text-xs font-black text-gray-700 hover:bg-gray-50"
                        >
                            Categories
                        </Link>
                        <Link
                            href={route('admin.research-tips.create')}
                ...
                <form onSubmit={submitSearch} className="mt-4">
                    <div className="relative">
                        ...
                        <input
                            ...
                            placeholder="Search by title, abstract, author, category..."
                    ...
                    <table className="min-w-full divide-y divide-gray-100">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-[11px] font-black uppercase tracking-widest text-gray-500">Published</th>
                            ...
                            {rows.length ? (
                                rows.map((t) => (
                                    <tr key={t.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3">
                                            ...
                                            <div className="text-xs text-gray-500 truncate">{t.abstract}</div>
                                        </td>
                                        <td className="px-4 py-3 text-sm text-gray-700">{t.author_name}</td>
                                        <td className="px-4 py-3 text-sm text-gray-700">{t.category}</td>
                                        <td className="px-4 py-3 text-sm text-gray-700">{t.published_date}</td>
                                ...
                                <tr>
                                    <td colSpan={5} className="px-4 py-10 text-center">
                                        <div className="text-sm font-black text-gray-900">No research tips found</div>
                                        <div className="mt-1 text-xs text-gray-500">Create your first research tip.</div>
                                    </td>
                                </tr>
                            )}
                        </tbody>
